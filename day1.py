...
data.assign(depth1 = lambda dataframe: dataframe['depth'].shift(-1)).assign(
    flag_inc = lambda dataframe: dataframe['depth'] < dataframe['depth1']).loc[:,'flag_inc'].sum()

data.assign(depth1 = lambda dataframe: dataframe['depth'].shift(1),
            depth2 = lambda dataframe: dataframe['depth'].shift(2)
            ).dropna().assign(
                sliding3      = lambda dataframe: dataframe['depth']+dataframe['depth1']+dataframe['depth2']).assign(
                sliding3shift = lambda dataframe: dataframe['sliding3'].shift(-1)).assign(
                    flag_inc = lambda dataframe: dataframe['sliding3'] < dataframe['sliding3shift']).loc[:,'flag_inc'].sum()
